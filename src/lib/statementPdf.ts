...
const statementSummaryItems = (totals: ReturnType<typeof calculateStatementTotals>) => [
  ...
  { label: 'Net Profit', value: usd(totals.shareable_net_amount), accent: true },
  { label: 'Dealer Share', value: usd(totals.dealer_share_amount) },
  { label: 'Company Share', value: usd(totals.company_share_amount) },
  { label: 'Printing Recovery', value: usd(totals.total_printing_costs) },
  { label: 'Shipping Recovery', value: usd(totals.total_shipping_costs) },
...
export async function downloadDealerAccountStatementPdf({
  ...
  if (rows.length === 0) {
    ...
    pdf.table(
      [
        { header: 'Period', width: 44 },
        { header: 'Payouts', width: 58, align: 'right' },
        { header: 'Store Exp', width: 52, align: 'right' },
        { header: 'Net Profit', width: 58, align: 'right' },
        { header: 'Dealer', width: 52, align: 'right' },
        { header: 'Company', width: 56, align: 'right' },
        { header: 'Cost Recovery', width: 66, align: 'right' },
        { header: 'Receivable', width: 62, align: 'right' },
        { header: 'Paid', width: 36, align: 'right' },
        { header: 'Remaining', width: 39, align: 'right' },
      ],
      rows.map(({ statement, totals }) => [
        statement.month,
        usd(totals.total_bank_payouts),
        usd(totals.total_store_expenses),
        usd(totals.shareable_net_amount),
        usd(totals.dealer_share_amount),
        usd(totals.company_share_amount),
        usd(totals.total_printing_costs + totals.total_shipping_costs),
        usd(totals.dealer_receivable_amount),
        usd(totals.paid_amount),
        usd(totals.remaining_amount),
      ]),
      { fontSize: 5.4, rowPadding: 5 },
    );
    pdf.summaryTable([
      { label: totalRemaining > moneyDisplayTolerance ? 'Net Amount Due' : 'Remaining Balance', value: usd(totalRemaining), accent: true },
    ]);
